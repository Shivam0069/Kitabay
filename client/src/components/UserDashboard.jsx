...
const UserDashboard = ({ setSelectedComponent }) => {
  ...
  useEffect(() => {
    let numberOfBorrowedBooks = userBorrowedBooks.filter(
      (book) => !book.returned
    );
    let numberOfReturnedBooks = userBorrowedBooks.filter(
      (book) => book.returned
    );
    setTotalBorrowedBooks(numberOfBorrowedBooks.length);
    setTotalReturnedBooks(numberOfReturnedBooks.length);
  }, [userBorrowedBooks]);
  const data = {
      ...
      <main className="relative flex-1 p-6 pt-28">
        ...
        <div className="flex flex-col-reverse xl:flex-row gap-2">
          ...
          <div className="flex flex-[4] flex-col gap-7 lg:gap-7 lg:py-5 justify-between xl:min-h-[85.5vh]">
            <div className="flex flex-col gap-7 flex-[4]">
              <div className="flex flex-col lg:flex-row  gap-7 overflow-y-hidden">
                <div
                  ...
                  className=" cursor-pointer flex items-center gap-3 bg-white p-5 max-h-[120px] overflow-y-hidden rounded-lg transition hover:shadow-inner duration-300"
                >
            ...
            <div className="bg-white p-7 text-lg sm:text-xl xl:text=3xl 2xl:text-4xl min-h-52 font-semibold relative flex-[3] fle  justify-center items-center rounded-2xl">
              <h4 className="overflow-y-hidden leading-10">
